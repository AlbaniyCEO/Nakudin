...
import { useGetCloudinarySignature } from "@workspace/api-client-react";
...
export function ImageUpload({ value, onChange, label = "Upload Image", aspect = "square" }: ImageUploadProps) {
  ...
  const getSignature = useGetCloudinarySignature();

  const handleFile = async (file: File) => {
    setUploading(true);
    setError(null);
    try {
      const sigData = await new Promise<{ signature: string; timestamp: number; apiKey: string; cloudName: string; uploadPreset: string }>((resolve, reject) => {
        getSignature.mutate({ data: {} }, {
