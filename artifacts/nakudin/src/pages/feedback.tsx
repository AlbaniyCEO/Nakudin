...
import { ArrowLeft, CheckCircle2, Loader2, MessageSquareText, Send } from "lucide-react";
...
export default function FeedbackPage() {
  ...
  return (
    <div className="px-4 py-4 pb-24 max-w-lg mx-auto" data-testid="page-feedback">
      <div className="flex items-center gap-3 mb-6">
        <Link href={user ? "/settings" : "/"}><button className="text-muted-foreground hover:text-foreground"><ArrowLeft size={22} /></button></Link>
        <div>
          <h1 className="text-xl font-bold text-foreground">Message Admin</h1>
          <p className="text-xs text-muted-foreground">Visitors, users, and shop owners can send feedback without registration.</p>
        </div>
      </div>

      <div className="surface-1 rounded-2xl p-4 mb-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {shop ? "Shop owner feedback will include your shop details automatically." : user ? "Your signed-in email will be attached." : "No account needed. Add your email only if you want Admin to know how to contact you."}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="surface-1 rounded-2xl p-4 space-y-4">
        {!user && (
        ...
        <div>
          ...
          <input value={subject} onChange={e => setSubject(e.target.value)} className="mt-1 w-full surface-2 border border-input rounded-lg px-3 py-2 text-sm text-foreground outline-none" placeholder="e.g. I need help verifying my shop" />
        </div>
        <div>
          <label className="text-xs text-muted-foreground">Message</label>
          <textarea value={message} onChange={e => setMessage(e.target.value)} rows={7} className="mt-1 w-full surface-2 border border-input rounded-lg px-3 py-2 text-sm text-foreground outline-none resize-none" placeholder="Write your message to Admin..." />
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        <Button type="submit" className="w-full" disabled={createFeedback.isPending}>
          {createFeedback.isPending ? <Loader2 className="animate-spin" size={16} /> : <><Send size={15} className="mr-2" />Send to Admin</>}
        </Button>
      </form>
    </div>
  );
